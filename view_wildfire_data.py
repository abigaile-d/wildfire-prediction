...
expander.markdown("There is no clear wildfire trend from 1992 to 2015. The wildfire incidents do not increase \
    year by year, and even dropped during 2007 to 2010, and 2012 to 2014. This is true for small and large fires.")
...
tmp_df = wildfire_df[['date', 'fire_size_class', 'state', 'incident', 'fire_size']]
if choice_display_fire and choice_display_state:
    tmp_df2 = tmp_df.groupby(['state', 'fire_size_class']).sum().reset_index()
    st.dataframe(tmp_df2, use_container_width=True)
    tmp_df2 = tmp_df2.pivot(index='state', columns='fire_size_class', values='incident')
    ...
    tmp_df2 = tmp_df.groupby(['state']).sum()
    tmp_df2 = tmp_df2.sort_values(by='incident', ascending=False)
    st.dataframe(tmp_df2, use_container_width=True)
...
if choice_display_state:
    tmp_df2 = tmp_df.groupby(['state']).sum().reset_index()
    tmp_df2['incident'] = tmp_df2['incident'] / tmp_df2['incident'].sum() * 100
    tmp_df2['incident'] = tmp_df2['incident'].round(decimals=2)
    chart = alt.Chart(tmp_df2).mark_arc(innerRadius=50, outerRadius=110).encode(
        theta=alt.Theta(field="incident", type="quantitative", title="% of occurence"),
...
if choice_display_fire or choice_display_state:
    ...
    expander.markdown("Only <3% of the total reported fire incidents are larger than 100 acres (class D & up), and <1% are \
        ...
        larger than 1000 acres  (class F & up) have occurred in California (CA), Idaho (ID), Arkansas (AK) \
        and Texas(TX) (each of these 4 states have ~10%).")
...
if choice_display_state:
    ...
    tmp_df2 = tmp_df.groupby([tmp_df.date.dt.year, 'state']).sum().reset_index()
    tmp_df2 = tmp_df2.pivot(index='date', columns='state', values='incident')
    st.bar_chart(tmp_df2)
    if choice_year == 'All':
        if choice_state == 'All':
            st.write("Displaying Top 5 U.S. States with Most Fires")
            top_states = tmp_df[['state', 'incident']].groupby('state').sum()
    ...
    tmp_df2 = tmp_df[['date', 'incident', 'state']].groupby(['date', 'state']).count()
    tmp_df2 = tmp_df2.reindex(pd.MultiIndex.from_product([list_dates, list_states], names=['date', 'state']), fill_value=0)
    tmp_df2 = tmp_df2['incident'].reset_index()
    tmp_df2.loc[tmp_df2['incident'] > 0, 'incident'] = 1
    tmp_df2 = tmp_df2.groupby(['incident', 'state']).count().reset_index()
    tmp_df2 = tmp_df2.pivot(index='state', columns='incident', values='date').sort_index(axis=1)
...
if len(choice_cause) > 0:
    # summary
    tmp_df = wildfire_df[['date', 'fire_size_class', 'state', 'stat_cause', 'incident']]
    ...
    tmp_df2 = tmp_df.groupby(['state', 'stat_cause']).sum().reset_index()
    tmp_df2 = tmp_df2.pivot(index='stat_cause', columns='state', values='incident')
    ...
    expander.markdown("Arson incidents seem to have decreased in the later years of the period (after 2007). \
        The trend of the rest of the top causes looks to have continued.")
    ...
    tmp_df2 = tmp_df.groupby(['state', 'stat_cause']).sum().reset_index()
    tmp_df2 = tmp_df2.pivot(index='state', columns='stat_cause', values='incident')
    st.bar_chart(tmp_df2)
    ...
    expander.markdown("The top causes of wildfires in California (CA) are miscellaneous and equipment use. \
        ...
        U.S. states with most wildfires.")
